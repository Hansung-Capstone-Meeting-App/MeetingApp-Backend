...
public interface CalendarControllerDocs {
    ...
    @Operation(
            ...
            description = "특정 이벤트를 로그인한 사용자의 Notion 캘린더 데이터베이스에 동기화합니다.\n\n" +
                    "이미 동기화된 이벤트는 동일 Notion 페이지를 갱신합니다 (중복 생성 방지).\n\n" +
                    "**사전 조건**\n" +
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "동기화 성공 — eventId, notionPageId, updated 반환"),
    ...
    @Operation(
            summary = "내 모든 워크스페이스 일정 Notion 일괄 동기화",
            description = "로그인 사용자가 멤버로 속한 모든 워크스페이스의 일정(팀 전체)을 Notion에 동기화합니다.\n\n" +
                    "이미 동기화된 이벤트는 동일 Notion 페이지를 갱신합니다 (중복 생성 방지).\n\n" +
                    "**사전 조건**\n" +
                    "- Notion 계정 연동 (`POST /api/oauth2/notion`)\n" +
                    "- 캘린더 DB 등록 (`PUT /api/oauth2/notion/calendar-database`)",
            responses = {
                    @ApiResponse(responseCode = "200", description = "동기화 성공 — workspaceCount, syncedCount, results 반환"),
                    @ApiResponse(responseCode = "400", description = "Notion 캘린더 DB 미등록"),
                    @ApiResponse(responseCode = "403", description = "Notion 미연동")
            }
    )
    ResponseEntity<?> syncAllMyWorkspaceEventsToNotion();
